// FLOOR GRID: lines on the figure's floor, one a metre apart and a heavier one every ten. The Monster Viewer's
// grid (MHGU-Monster-Viewer docs/render/grid.js, Raven, 2026-09-18: "Can we have a grid for the floor of the render
// space?"), brought over as it is -- Raven, 2026-09-19: "For now, Monster Viewer has a Grid Floor, I would like this
// to be in the Armor Viewer". A measuring aid of the viewer's own, not anything the game draws.
//
...
// are fixed in the world. The patch they are drawn on sits under the figure and fades out at a radius set by its
// size (15 m at the least, which is what a hunter or a Palico gets).
...
// reach(): the figure's horizontal half-size, which sets the patch's radius. All three are read every frame.
export function createFloorGrid({ scene, floor, centre, reach }){
  const material = new THREE.ShaderMaterial({
    ...
    // pulled toward the camera, so it lies on top of anything else at the same height instead of fighting it
    polygonOffset: true, polygonOffsetFactor: -1, polygonOffsetUnits: -4,
  });
  const mesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), material);
  mesh.name = 'floor-grid';
  mesh.rotation.x = -Math.PI / 2;
  mesh.renderOrder = -997;                 // before the figure's translucent materials
